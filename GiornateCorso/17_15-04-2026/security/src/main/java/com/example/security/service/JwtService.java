...
public class JwtService {
    ...
    /**
     * Genera un refresh token per l'utente autenticato.
     * Il refresh token ha una scadenza più lunga del token normale.
     */
    public String generateRefreshToken(UserDetails user) {
        // Refresh token dura 7 giorni (7 * 24 * 60 * 60 * 1000 ms)
        long refreshExpiration = 7 * 24 * 60 * 60 * 1000L;
        return Jwts.builder()
            .setSubject(user.getUsername()) // Username come subject
            .setIssuedAt(new Date()) // Data di emissione
            .setExpiration(new Date(System.currentTimeMillis() + refreshExpiration)) // Data di scadenza (7 giorni)
            .signWith(Keys.hmacShaKeyFor(secret.getBytes()), SignatureAlgorithm.HS256) // Firma del token
            .compact(); // Compatta il tutto in una stringa JWT
    }
    /**
